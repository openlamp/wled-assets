...
| WLED 이름 | 번역 | 설명 |
|---|---|---|
| `Reverse` | 반전 | 효과를 세그먼트에서 반대 방향으로 재생. |
| `Mirror` | 미러 | 세그먼트를 나눠 중앙에서 대칭. |
| `Freeze` | 정지 | 현재 프레임에서 효과 정지. |
| `On/Off` | 켜기/끄기 | 세그먼트 켜기/끄기. |
| `Transition` | 전환 | 상태/프리셋 전환 시 크로스페이드 시간. |
| `Sound sim` | 오디오 시뮬 | 실시간 오디오가 없을 때 오디오 시뮬레이션. |
